...
def manual_navigation():
    ...
    # Step 2: Turn left (1.5 s)
    turn_left(motor_left, motor_right, turning_speed=60, turn_time=1.5)

    # Step 3: LINE FOLLOWING instead of blind forward 1.1 m
    step3_speed = 70
    base_speed_m_per_s = 0.3 * (step3_speed / 60)  # = 0.2 m/s
    step3_time_s = 10 / base_speed_m_per_s       # ≈ 5.5 s
    ...
    # Step 5: Forward 3.0 m (still time-based)
    move_forward(motor_left, motor_right, speed=60, distance_m=3.0)

    # Step 6: Turn right (your original code had turn_right here)
    turn_right(motor_left, motor_right, turning_speed=40, turn_time=1.0)

    # Step 7: Forward 0.1 m
    move_forward(motor_left, motor_right, speed=60, distance_m=2.0)
